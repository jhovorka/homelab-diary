...
{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/main.tf" commit="blog/homelab-diary-part4" lines="1-34" >}}

That unsequenced part is a deliberate choice, not an oversight. A config change on a control plane can restart kube-apiserver, controller-manager, and scheduler on that node - not always, depending on which part of the config actually changed, but the module has no way to know that in advance. Applying to every control plane at once therefore isn't strictly risk-free. I went back and forth on this: an earlier version of this module sequenced control plane config application one node at a time, gated on etcd staying healthy between each, the same way the upgrade resource further down still does. In practice that turned out to be a lot of extra script for a risk that mostly shows up during an actual OS upgrade - which already gets the careful treatment - rather than during day-to-day config changes, where I generally know what I'm about to change. So I simplified it back down: only upgrades get the sequencing and health gate now: for everything else, I'm expected to know what I'm doing before I hit apply.
...
{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/main.tf" commit="blog/homelab-diary-part4" lines="36-53" >}}
...
That's on purpose. The provider has no native upgrade resource, so this shells out to `talosctl upgrade` directly, and upgrading a node means rebooting it - so nodes need to go one at a time, not all at once, otherwise multiple control planes rebooting together would put etcd's quorum at risk. The script's `local-exec` provisioner loops over every node in `upgrade_order` - control planes first, both groups sorted for a stable sequence across applies. For each node, it checks whether it's already running the target version and skips it if so, otherwise it runs `talosctl upgrade --wait=false` and polls `talosctl version` until the node comes back up on the target version.
...
{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/main.tf" commit="blog/homelab-diary-part4" lines="86-198" >}}
...
Right after that comes `control_plane_ips`, `worker_ips`, and `upgrade_order` - the last one is just the first two concatenated, control planes first, each sorted independently for a stable order across plans. This is what `upgrade` loops over later to decide which node goes first: control planes before workers, so etcd quorum is never the thing left exposed while some node reboots partway through an OS upgrade.

{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/locals.tf" commit="blog/homelab-diary-part4" lines="15-26" >}}
...
{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/locals.tf" commit="blog/homelab-diary-part4" lines="60-126" >}}
