...
{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/main.tf" commit="blog/homelab-diary-part4" lines="10-45" >}}

Control planes don't get the same treatment, because any machine config change on a control plane - not just a Talos or Kubernetes version bump - restarts kube-apiserver, controller-manager, and scheduler on that node. Applying config to every control plane at once risks the exact same thing an unsequenced OS upgrade would: multiple control planes going down together and taking etcd's quorum with them. `talos_machine_configuration_apply` can't be sequenced the way the `upgrade` resource further down is, either, for a structural reason: a resource with a `for_each` can't depend on another instance of itself, since Terraform has to fully expand the `for_each` before it can resolve any single instance's dependencies - a same-resource dependency chain is a cycle by construction, not just a style choice. So control planes go through `terraform_data.control_plane_config_apply` instead: one resource for the whole cluster, not one per node, that shells out to `talosctl apply-config` directly inside a loop, one control plane at a time, confirming etcd is healthy across every control plane before moving to the next.
...
Right after that comes `control_plane_ips`, `worker_ips`, and `upgrade_order` - the last one is just the first two concatenated, control planes first, each sorted independently for a stable order across plans. This is what both `control_plane_config_apply` and `upgrade` loop over later to decide which node goes first: control planes before workers, so etcd quorum is never the thing left exposed while some other node reboots or gets reconfigured.
