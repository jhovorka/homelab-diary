...
This is the last module I need for this part, and it's also the one that actually turns a pile of VMs into a working Talos Kubernetes cluster, so there is quite a lot to unpack here. The module generates the cluster's machine secrets, renders and applies a machine config for every node, bootstraps the cluster on the first control plane node, waits for it to report healthy, and keeps every node's Talos version in sync with the installer image I give it. This is what it looks like:

{{< github repo="hovorka-labs/iac-modules" path="terraform/modules/talos/main.tf" commit="blog/homelab-diary-part4" >}}

There is a clear pipeline hiding in there, even though it's not obvious at first glance because most of it is just resource blocks sitting next to each other. talos_machine_secrets generates the secrets shared by the whole cluster, talos_machine_configuration renders a config for each node, talos_machine_configuration_apply pushes it to the node, talos_machine_bootstrap bootstraps Kubernetes on the first control plane node, talos_cluster_health waits until the provider considers the cluster healthy, and only then does the upgrade logic and talos_cluster_kubeconfig kick in. None of this ordering is enforced by data flowing between the resources on its own, Terraform just sees a bunch of resources depending on the same client_configuration, so most of the depends_on blocks you see here exist purely to force that sequence.
